...
>
        ...
        <div class="flex items-center justify-between gap-x-2 sm:gap-x-3 h-16 w-full">
            ...
            <div class="hidden xl:flex self-stretch shrink-0">
                <x-nav-link href="{{ route('genres') }}" :active="request()->routeIs('publishers') || request()->routeIs('genres')">
                    <x-pixelarticon name="directions" :size="24" class="mr-1.5" />
                    {{ __('Explore') }}
                </x-nav-link>
            </div>
            ...
            <div class="relative flex-1 min-w-0 xl:flex-none xl:w-96 flex items-center h-[2.2rem]">
                ...
                <x-pixelarticon name="search" :size="20" class="absolute mx-2 text-cod-gray-300 pointer-events-none" />
                <x-pixelarticon name="close" :size="20" wire:click="clearSearchResults" class="absolute right-0 mx-2 text-cod-gray-300 cursor-pointer" />
            </div>
            ...
            <div class="hidden xl:block relative shrink-0">
                <x-dropdown align="right" width="60">
                        ...
                        @else
                            <span class="inline-flex rounded-md mt-0.5">
                                <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-xl leading-4 font-medium rounded-md text-cod-gray-500 dark:text-cod-gray-400  hover:text-cod-gray-700 dark:hover:text-cod-gray-300 focus:outline-none focus:bg-cod-gray-50 dark:focus:bg-cod-gray-700 active:bg-cod-gray-50 dark:active:bg-cod-gray-700 transition ease-in-out duration-150">
                                    ...
                                    <x-pixelarticon name="chevron-down" :size="22" class="ml-2 -mr-0.5" />
                                </button>
                            </span>
                        @endif
                    </x-slot>
                    ...
                    <x-slot name="content">
                        <!-- Account Management -->
                        <div class="block whitespace-nowrap px-4 py-2 text-cod-gray-400 sepia_">
                            {{ __('Manage Account') }}
                        </div>
                        ...
                        <x-dropdown-link href="{{ route('my-saves') }}" wire:navigate>
                            <span class="inline-flex items-center gap-x-2">
                                <x-pixelarticon name="save" :size="20" />
                                {{ __('My Saves') }}
                            </span>
                        </x-dropdown-link>
                        ...
                        <button
                            type="button"
                            class="block w-full whitespace-nowrap px-4 py-2 text-left text-xl leading-5 text-cod-gray-700 dark:text-cod-gray-300 hover:bg-cod-gray-100 dark:hover:bg-rose-600 dark:hover:text-white focus:outline-none focus:bg-cod-gray-100 dark:focus:bg-cod-gray-900 transition duration-150 ease-in-out sepia_"
                            x-data="{
                        ...
                        >
                            <span class="inline-flex items-center gap-x-2">
                                <x-pixelarticon name="cursor" :size="20" />
                        ...
                        @if (Laravel\Jetstream\Jetstream::hasApiFeatures())
                            <x-dropdown-link href="{{ route('api-tokens.index') }}">
                                <span class="inline-flex items-center gap-x-2">
                                    <x-pixelarticon name="script" :size="20" />
                                    {{ __('API Tokens') }}
                                </span>
                            </x-dropdown-link>
                        @endif
                        ...
                        @if (Auth::user()->hasRole('admin'))
                            <x-dropdown-link href="{{ route('admin.games') }}">
                                <span class="inline-flex items-center gap-x-2">
                                    <x-pixelarticon name="gamepad" :size="20" />
                                    {{ __('Game Manager') }}
                                </span>
                            </x-dropdown-link>
                            <x-dropdown-link href="{{ route('admin.fonts') }}">
                                <span class="inline-flex items-center gap-x-2">
                                    <x-pixelarticon name="font" :size="20" />
                                    {{ __('Font Manager') }}
                                </span>
                            </x-dropdown-link>
                        ...
                        <x-dropdown-link href="{{ route('about') }}">
                            <span class="inline-flex items-center gap-x-2">
                                <x-pixelarticon name="info-box" :size="20" />
                                {{ __('About') }}
                            </span>
                        </x-dropdown-link>
                        ...
                        <form method="POST" action="{{ route('logout') }}" x-data>
                            ...
                            <a href="{{ route('logout') }}" class="block w-full whitespace-nowrap px-4 py-2 text-left text-xl leading-5 text-cod-gray-700 dark:text-cod-gray-300 hover:bg-cod-gray-100 dark:hover:bg-rose-600 dark:hover:text-white focus:outline-none focus:bg-cod-gray-100 dark:focus:bg-cod-gray-900 transition duration-150 ease-in-out sepia_"
                                    @click.prevent="$root.submit();">
                                <span class="inline-flex items-center gap-x-2">
                                    <x-pixelarticon name="logout" :size="20" />
                                    {{ __('Log Out') }}
                                </span>
                            </a>
                        </form>
            ...
            <div class="hidden xl:flex self-stretch shrink-0">
                <x-nav-link href="{{ route('login') }}" :active="request()->routeIs('login')">
                    <x-pixelarticon name="login" :size="24" class="mr-1.5" />
                    {{ __('Login') }}
                </x-nav-link>
            </div>
            @endguest

            <div class="hidden xl:flex items-center self-stretch shrink-0">
                <x-theme-switcher />
            </div>

            <!-- Hamburger -->
            <div class="flex items-center shrink-0 xl:hidden">
    ...
    <!-- restore notification banner (below fixed nav; keep under toasts z-[200]) -->
    ...
    <div
        ...
        class="relative z-[90] w-full bg-amber-500/90 dark:bg-amber-600/90 text-amber-950 dark:text-amber-50 text-base py-2 px-4 flex items-center justify-between gap-x-4"
    >
    ...
    >
        <div>
            ...
            <x-responsive-nav-link href="{{ route('genres') }}" :active="request()->routeIs('genres') || request()->routeIs('publishers')">
                <span class="inline-flex items-center gap-x-2">
                    <x-pixelarticon name="directions" :size="20" />
                    {{ __('Explore') }}
                </span>
            </x-responsive-nav-link>
        </div>
        ...
        <div>
            ...
            <div>
                <div @click="cycleTheme()" class="block w-full pl-3 pr-4 py-2 border-l-4 border-transparent dark:border-transparent hover:border-cod-gray-300 hover:dark:border-rose-600 dark:border-cod-gray-600 text-left text-lg text-cod-gray-600 dark:text-cod-gray-400 hover:text-cod-gray-700 hover:dark:text-cod-gray-300 hover:bg-cod-gray-50 hover:dark:bg-rose-900/50 focus:outline-none focus:text-cod-gray-800 dark:focus:text-cod-gray-200 focus:bg-cod-gray-100 dark:focus:bg-cod-gray-900 focus:border-cod-gray-700 dark:focus:border-cod-gray-300 smooth-300 cursor-pointer">
                    <div class="flex items-center gap-x-2">
                        <span x-cloak x-show="theme === 'light'" class="inline-flex items-center gap-x-2">
                            <x-pixelarticon name="sun" :size="20" />
                            {{ __('Light theme') }}
                        </span>
                        <span x-cloak x-show="theme === 'dark'" class="inline-flex items-center gap-x-2">
                            <x-pixelarticon name="moon" :size="20" />
                            {{ __('Dark theme') }}
                        </span>
                        <span x-cloak x-show="theme === 'system'" class="inline-flex items-center gap-x-2">
                            <x-pixelarticon name="computer" :size="20" />
                            {{ __('System theme') }}
                        </span>
                    </div>
                </div>
                ...
                @if (Auth::user()->hasRole('admin'))
                    ...
                    <x-responsive-nav-link href="{{ route('admin.games') }}" :active="request()->routeIs('admin.games')">
                        <span class="inline-flex items-center gap-x-2">
                            <x-pixelarticon name="gamepad" :size="20" />
                            {{ __('Game Manager') }}
                        </span>
                    </x-responsive-nav-link>
                    <x-responsive-nav-link href="{{ route('admin.fonts') }}" :active="request()->routeIs('admin.fonts')">
                        <span class="inline-flex items-center gap-x-2">
                            <x-pixelarticon name="font" :size="20" />
                            {{ __('Font Manager') }}
                        </span>
                    </x-responsive-nav-link>
                    <x-responsive-nav-link href="{{ route('admin.backup') }}" :active="request()->routeIs('admin.backup')">
                        <span class="inline-flex items-center gap-x-2">
                            <x-pixelarticon name="save" :size="20" />
                            {{ __('Backup / Restore') }}
                        </span>

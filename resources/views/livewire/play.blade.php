<x-container class="h-screen -mt-[4rem]">
    <div class="flex flex-col gap-y-8">
        ...
        <div class="flex flex-col gap-x-0 gap-y-1 xl:gap-x-1 xl:gap-y-0 xl:flex-row sticky top-[4rem] items-start justify-between rounded-md overflow-hidden border-[4px] xl:border-0 border-cod-gray-800/50 shadow-xl shadow-black">
            ...
            <!-- user data -->
            <div class="xl:w-[40%] xl:flex xl:flex-col w-full mb-4">
                <!-- tabs -->
                    ...
                    <x-accordion wire:click="toggle('screenshots')" :toggler="$accordion_toggler['screenshots']">
                        ...
                        <div class="flex pb-6 overflow-hidden overflow-x-auto flex-col">
                            <div class="flex flex-no-wrap gap-x-2">
                                ...
                                <img @click="$dispatch('open-fixed-modal')" wire:click="screenshot({{ $key }})" src="{{ $img }}" alt="{{ $key }}" class="rounded-md w-[10rem] h-[6rem] brightness-75 hover:brightness-100 smooth-300 cursor-pointer">
                                @empty
                                <div class="flex flex-col cursor-default rounded-xl py-6 w-full fade text-cod-gray-300">
        ...
        <!-- screenshots modal -->
        <x-fixed-modal :width="65" :height="75" :loader="$current_screenshot_key === -1">
            ...
            <div class="px-6">
                <div class="flex items-center justify-between gap-x-4">
                    <div wire:click="changeScreenShot('left')" class="p-1">
                        <x-icons.arrow-left class="w-10 h-10 text-cod-gray-200 hover:text-rose-500 smooth-300 cursor-pointer" />
                    </div>
                    <div class="w-[100vw] h-[60vh]">
                        <img class="mb-4 w-full h-full rounded-md" src="{{ $game['screenshots'][$current_screenshot_key] }}" alt="Screenshot: {{ $game['title'] }}">
                    </div>
                    <div wire:click="changeScreenShot('right')" class="p-1">
                        <x-icons.arrow-right class="w-10 h-10 text-cod-gray-200 hover:text-rose-500 smooth-300 cursor-pointer" />
                    </div>
                </div>
            </div>
            @endif
